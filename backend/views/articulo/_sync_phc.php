					...
					<tbody>
						...
						<?php if (  !strcmp(  ($art_status =  !isset($item['model']) ? 'danger' : ( ($item['dbmodel']->precio < $item['model']->precio) ?'success':'warning' ) ), $filter ) || !$filter  ) :?>
    						...
    						<tr class="<?=$art_status  ?>" >
    						     ...
    						     <td><?= isset($item['dbmodel']->moneda)?$item['dbmodel']->moneda:'' ?> &nbsp;
    						     <?=  (isset($item['dbmodel']->moneda) && !strcmp($item['dbmodel']->moneda, 'USD') )? Yii::$app->formatter->asCurrency( $item['dbmodel']->precio ):''?>  </td>
